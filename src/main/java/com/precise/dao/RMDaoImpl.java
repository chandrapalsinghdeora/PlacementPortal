...
@Repository
public class RMDaoImpl extends ConnectionDao implements RMDao {
	...
	public List<HotList> getHRHotListedDataByRole(int cmpRoleId) {
		...
		List<HotList> shortList = new ArrayList<HotList>();
		String procedureCall = "{call Sp_hotList(?,?,?,?,?,?,?,?)}";
		Connection connection = null;
		try {
			connection = getJdbcTemplate().getDataSource().getConnection();
			CallableStatement callableSt = connection.prepareCall(procedureCall);
			callableSt.setString(1, "SelecthotListForHR");
			callableSt.setInt(2, 0);
			callableSt.setInt(3, 0);
			callableSt.setInt(4, 0);
			callableSt.setBoolean(5, true);
			...
			while (rs.next()) {
				...
				HotList shortlist = new HotList();
				shortlist.setApplyId(rs.getInt("Pk_ApplyId"));
				shortlist.setShortListId(rs.getInt("PK_HotListId"));
				shortlist.setEmailID(rs.getString("EmailID"));
				shortlist.setName(rs.getString("Name"));
				shortlist.setPreference(rs.getInt("Preference"));
				shortlist.setRollNumber(rs.getInt("RollNumber"));
				shortlist.setStatus(rs.getString("Status"));
				shortlist.setShortList(rs.getBoolean("IsHostList"));
				shortlist.setCmpRoleId(rs.getInt("Fk_RoleId"));
				shortList.add(shortlist);
		...
		//System.out.println("RMDaoImpl.getShortlistedDataByRole()");
				...
				try {
					...
					while (rs.next()) {
						...
						shortlist.setStatus(rs.getString("Status"));
						shortlist.setShortList(rs.getBoolean("IsShortList"));	
						shortlist.setShortListBy(rs.getString("ShortListBy"));
						shortList.add(shortlist);
						//System.out.println("get shortlistedd---- " + rs.getInt("Pk_ApplyId"));
					}
				} catch (SQLException e) {
...
/*
		...
		try {
			...
			for (CloseStatus closeStatus : downloadCvlist) {
				...
				if (rs.next()) {
					...
					fileNameList.add(rs.getString("RollNumber")+"_"+rs.getString("CVName")+"###"+rs.getString("ResumePath"));
	...
	public List<String> getRMDownloadCoverList(List<CloseStatus> downloadCvlist) {
		...
		try {
			...
			for (CloseStatus closeStatus : downloadCvlist) {
				...
				//System.out.println(rs.getString("CVName")+" :: "+rs.getString("ResumePath")+" ,,,, "+"Cover letter :: "+rs.getString("CoverTitle")+" "+rs.getString("CoverPath"));
				if(rs.getString("CoverPath")!=null){
					//if(fileNameList.contains(rs.getString("CoverTitle")+"###"+rs.getString("CoverPath")))
						
						//fileNameList.add(rs.getString("CoverTitle")+"1"+"###"+rs.getString("CoverPath"));
						System.out.println(rs.getString("CVName")+"_"+rs.getString("CoverTitle")+"###"+rs.getString("CoverPath"));
					fileNameList.add(rs.getString("CVName")+"_"+rs.getString("CoverTitle")+"###"+rs.getString("CoverPath"));
					//else
						//fileNameList.add(rs.getString("CoverTitle")+"###"+rs.getString("CoverPath"));
				}
				// Pk_ApplyId,Title,,CvFile
			}*/
			if (rs.next()) {
				...
				//System.out.println(rs.getString("CVName")+" :: "+rs.getString("ResumePath")+" ,,,, "+"Cover letter :: "+rs.getString("CoverTitle")+" "+rs.getString("CoverPath"));
				if(rs.getString("CoverPath")!=null){
					/*if(fileNameList.contains(rs.getString("RollNumber")+"_"+rs.getString("CVName")+"_CoverTittle###"+rs.getString("CoverPath")))
					{*/
						fileNameList.add(rs.getString("RollNumber")+"_"+rs.getString("CVName")+"_CoverLetter###"+rs.getString("CoverPath"));
					/*}else
						fileNameList.add(rs.getString("RollNumber")+"_"+rs.getString("CVName")+"_CoverTittle###"+rs.getString("CoverPath"));
				*/}
				// Pk_ApplyId,Title,,CvFile
			}
	...
	public void hotlistRMRelease(HotList hotlist) {
		...
		List<Integer> hotistedIds = hotlist.getHotListIdslist();
		Connection connection = null;
		...
		try {
			connection = getJdbcTemplate().getDataSource().getConnection();
			callableInsert = connection.prepareCall("{call Sp_hotList(?,?,?,?,?,?)}");
			for (int i = 0; i < hotistedIds.size(); i++) {
				callableInsert.setString(1, "hotListReleaseRM");		
				callableInsert.setInt(2, hotistedIds.get(i));
				callableInsert.setInt(3, 0);
	...
	public String selectUsersInformationByRM(int applyId,String studentGreetingSubject, String greetings, int userID, String sender_email) {
		...
		try{
			...
			CallableStatement callableInsert = connection.prepareCall("{call Sp_hotList(?,?,?,?,?)}");
				...
				CallableStatement callableUpdate = connection.prepareCall("{call Sp_ShortList(?,?,?,?,?,?,?)}");
				/*System.out.println("apply id::"+applyIdListIdslist.get(i));*/
	...
	public List<ShortList> getAllRM(int compRoleId) {
		...
		try {
			...
			String procedureCall1 = "{call Sp_ShortList(?)}";
	...
	public boolean getReleaseFlagStatus(int role) {
		boolean flag=false;
		final String procedureCall = "{call Sp_ShortList(?,?)}";
		Connection connection = null;
		...
		try {
			...
			callableSt.setInt(2, role);
			ResultSet rs = callableSt.executeQuery();
			if (rs.next()) {
				flag=rs.getBoolean("flag");
				System.out.println(flag+"gsyagdsuyagd");
	...
	public List<HotList> getFinalOffer(int cmpRoleId) {
		System.out.println("RMDaoImpl.getHRShortlistedDataByRole())))))))))))))"+cmpRoleId);
		List<HotList> finaloffer = new ArrayList<HotList>();
		String procedureCall = "{call Sp_hotList(?,?)}";
		Connection connection = null;
		try {
			connection = getJdbcTemplate().getDataSource().getConnection();
			CallableStatement callableSt = connection.prepareCall(procedureCall);
			callableSt.setString(1, "finalOffer");
			...
			while (rs.next()) {
				...
				shortlist.setName(rs.getString("Name"));
				shortlist.setRollNumber(rs.getInt("RollNumber"));
				shortlist.setStatus(rs.getString("CompanyStatus"));
				//shortlist.setCmpRoleId(rs.getInt("Fk_RoleId"));
				finaloffer.add(shortlist);
			System.out.println("get shortlistedd---- " +finaloffer.toString());
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
