...
@Repository("closeStatusRepo")
public class CloseStatusDAOImpl implements CloseStatusDAO {
	...
	public CloseStatus getCloseStatusHeader(int cmpRoleId) {
		...
		try {
			...
			if (rs.next()) {
				...
				closeStatus.setPrefrences(rs.getInt("PrefenceSurvey"));
				...
				closeStatus.setHrHotlist(rs.getBoolean("IsHRHotList"));
				closeStatus.setNoOfSelectionRounds(rs.getString("NoOfSelectionRound") == null ? 0:rs.getInt("NoOfSelectionRound"));
			}
	...
	public String getUserData(String shortListId,int compRoleId, boolean rankFlag) {
		...
		String table="";
		if(rankFlag)
			 table="<table border='1'><tr><th>Email ID</th><th>Name</th><th>Preference</th></tr>";
		else
			table="<table border='1'><tr><th>Email ID</th><th>Name</th></tr>";
		try {
			/*String[] idArr = shortListId.split(",");
			for (String id : idArr) {
				System.out.println("id:::::::::::::::::::"+id);*/
	...
	public String getSelectedUserHotList(String hotListId,int compRoleId){
		...
		try {
			...
			callableStmt.setString(1, "HotlistedByRM");
			callableStmt.setInt(2,0);
			callableStmt.setInt(3, 0);
			callableStmt.setInt(4, 0);
			callableStmt.setBoolean(5, true);
			callableStmt.setString(6, hotListId);
			callableStmt.setBoolean(7,true);
			callableStmt.setInt(8, compRoleId);
			ResultSet rs = callableStmt.executeQuery();
			System.out.println("result set ::: "+rs);
	...
	public String getSelectedUserHRList(String shortListId,int compRoleId){
		...
		try {
			...
			System.out.println("inside getSelectedUserHRList method::");
			e.printStackTrace();
		}
		return table;
	}
	...
	public String getSelectedUserHRHotList(String shortListId,int compRoleId){
		...
		try {
				...
				connection = getJdbcTemplate().getDataSource().getConnection();
				CallableStatement callableStmt = connection.prepareCall("{call Sp_hotList(?,?,?,?,?,?,?,?)}");
				callableStmt.setString(1, "HotlistedByHR");
				callableStmt.setInt(2,0);
				callableStmt.setInt(3, 0);
				callableStmt.setInt(4, 0);
				callableStmt.setBoolean(5, true);
