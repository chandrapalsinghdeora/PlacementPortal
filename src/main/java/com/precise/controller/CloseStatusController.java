...
@Controller
public class CloseStatusController {
	...
	@RequestMapping(value ="/getAllCloseStatusDetails",method = RequestMethod.GET)
	public ModelAndView getAllCloseStatusDetails(ModelAndView model,HttpServletRequest req,Model model1){
		...
		int cmpRoleId=0;
		if(req.getParameter("cmpRoleId")!=null){
		...
		System.out.println("getAllCloseStatusDetails() ::  cmpRoleId  "+cmpRoleId);
		List<CloseStatus> CloseStatusDetaillist = closeStatusService.getCloseStatusDetail(cmpRoleId);
		//System.out.println("size CloseStatusDetaillist :"+CloseStatusDetaillist.size());
		req.setAttribute("CloseStatusDetails", CloseStatusDetaillist);
		
		List<CloseStatus> approveCloseStatusList = closeStatusService.getApproveCloseStatusList(cmpRoleId);
		//System.out.println("size approveCloseStatusList :"+approveCloseStatusList.size());
		req.setAttribute("approveCloseStatusList", approveCloseStatusList);
		
		List<CloseStatus> unApproveCloseStatusList = closeStatusService.getUnApproveCloseStatusList(cmpRoleId);
		//System.out.println("size unApproveCloseStatusList :"+unApproveCloseStatusList.size());
		req.setAttribute("unApproveCloseStatusList", unApproveCloseStatusList);
		
		List<ShortList> shortlist=rmService.getShortlistedDataByRole(cmpRoleId);
		//System.out.println("size shotlist-"+shortlist.size());
		req.setAttribute("shortlist",shortlist);
		boolean releaseFlag=rmService.getReleaseFlagStatus(cmpRoleId);	
		if(releaseFlag)
		...
		//System.out.println("size viewReleasedShortlist-"+viewReleasedShortlist.size());
		req.setAttribute("viewReleasedShortlist",viewReleasedShortlist);
		
		List<HotList> hotlist=hotListservice.getShortlistedDataByRole(cmpRoleId);//role
		//System.out.println("size hotlist-"+shortlist.size());	 	
 		req.setAttribute("hotlist",hotlist);
 		
 		//List<HotList> hotlistRelease=hotListservice.getHotReleaseCompany(cmpRoleId);//role
 		List<HotList> viewHotReleased=superRMService.getViewReleasedHotlist(cmpRoleId);//role 	
 		//System.out.println("size viewHotReleased-"+viewHotReleased.size());
 		req.setAttribute("hotlistRelease",viewHotReleased);
 	
 		List<ShortList> shortlistedByHr=rmService.getHRShortlistedDataByRole(cmpRoleId);
		//System.out.println("size shortlistedByHr-"+shortlistedByHr.size());
		req.setAttribute("shortlistedByHr",shortlistedByHr);
		
		List<HotList> hotlistedByHr=rmService.getHRHotListedDataByRole(cmpRoleId);
		//System.out.println("hortlistedByHr-"+hotlistedByHr.size());
		req.setAttribute("hotlistedByHr",hotlistedByHr);
		
		List<ShortList> shortlistone = rmService1.getShortlistedDataByRole(cmpRoleId);
		req.setAttribute("shortlistOne", shortlistone);
		//System.out.println("size shortlistone-" + shortlistone.size());
		
		List<HotList> finalOffer=rmService.getFinalOffer(cmpRoleId);
		//System.out.println("hortlistedByHr-"+hotlistedByHr.size());
		req.setAttribute("finalOffer",finalOffer);
		...
		model.setViewName("CloseStatus");
		return model;
		
	}
	
	@RequestMapping(value ="/getAllCloseStatusDetailsForSuperRM1",method = RequestMethod.GET)
	public ModelAndView getAllCloseStatusDetailsForSuperRM(ModelAndView model,HttpServletRequest req,Model model1){
		...
		req.setAttribute("closeStatus", header);
		model1.addAttribute("cmpRoleId",cmpRoleId);
		model.setViewName("CloseStatusForSuperRM1");
		return model;
		
	}
	
	@RequestMapping(value ="/getAllCloseStatusDetailsForSuperRM2",method = RequestMethod.GET)
	public ModelAndView getAllCloseStatusDetailsForSuperRM2(ModelAndView model,HttpServletRequest req,Model model1){
		...
		List<HotList> hotlist=hotListservice.getShortlistedDataByRole(cmpRoleId);//role
 		...
 		System.out.println("list hotlistRelease-"+hotlistRelease.size());
		...
		req.setAttribute("closeStatus", header);
		model1.addAttribute("cmpRoleId",cmpRoleId);
		model.setViewName("CloseStatusForSuperRM2");
		return model;
		
	}
	
	@RequestMapping(value="/getUserValues")
	...
	@RequestMapping(value="/getInternshipList" ,method = RequestMethod.POST)
	...
	@RequestMapping(value="/getCVList" ,method = RequestMethod.POST)
	...
	@RequestMapping(value="/getInfoValues")
	...
	@RequestMapping(value="/getInfoValuesWithoutSession")
	public void  getInfoValuesWithoutSession(HttpSession session,HttpServletRequest req,HttpServletResponse res){
		...
		InfoCloseStatus userValue = closeStatusService.getInfoValues(cmpRoleId);
		JSONArray jsonArray = new JSONArray();
		JSONObject infoDetailsJson = new JSONObject();
		infoDetailsJson.put("clusterName", userValue.getClusterName());
		infoDetailsJson.put("cohortName", userValue.getCohortName());
	...
	public String updateFlag(HttpServletRequest request) {
		...
		closeStatusService.updateFlag(closeStatuslist);
		return "redirect:getAllCloseStatusDetails";
	}
	
	@RequestMapping(value = "/updateFlagBySuperRM1", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/updateFlagBySuperRM2", method = RequestMethod.POST)
	...
	public String saveShortList(HttpServletRequest request, CloseStatus closeStatus) {
		...
		closeStatusService.saveHRList(closeStatus.getEmailId(),cmpRoleId,closeStatus.getMailDes(),emailId,closeStatus.getHrName(),userid);
	...
	@RequestMapping(value = "/shortListBySuperRM1", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/shortListBySuperRM2", method = RequestMethod.POST)
	...
	public void zipFiles(HttpServletResponse response, HttpServletRequest request) throws IOException {
		...
		List<String> downloadFileNameList = closeStatusService.getDownloadCVList(downloadCvlist);
	...
	public String generateMailFormat(String emailId,String hrName,String link,String shortList,Boolean rankFlag,String msgDesc,int companyRoleId,String rmemailId){
		...
		if(rankFlag!=null && rankFlag){
			prefernceValue=closeStatusService.getSelectedUserValues(shortList,companyRoleId, rankFlag)+"<br/>";
			mailHtml.append("<html><body>Dear "+hrName+",<br/><br/> "+msgDesc+"<br/>Prefernce List :<br/></br>"+prefernceValue+"<br/>Please <a href="+link+">Click here</a> for managing applications.");
		}else{
			prefernceValue="";
			mailHtml.append("<html><body>Dear "+hrName+",<br/><br/> "+msgDesc+"<br/>Please <a href="+link+">Click here</a> for managing applications.");
		}
	...
	@RequestMapping(value = "/pdfmerge", produces = "application/pdf")
	...
	public String getPDFBinder(List<File> files, List<String> fileNameList, String pdfBinderLocation) throws Exception {
		...
		for (int i = 0; i < files.size(); i++) {
			file1 = files.get(i);
			//fileName = fileNameList.get(i);
			if(FilenameUtils.getExtension(file1.getName()).equals("pdf")){
				doc = PDDocument.load(file1);
				PDFmerger.addSource(file1);
				doc.close();
			}else{
				
				System.err.println("File Not Found "+file1.getName());
			}
		}
