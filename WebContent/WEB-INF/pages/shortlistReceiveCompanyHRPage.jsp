...
<div class="container min-height">
         ...
         <td> ${shrt.name} </td>
          ...
          <td align="center"> 
            ...
            <c:if test="${shrt.shortList eq false}"> 
             <input type="checkbox" name="shortlisted" value="${shrt.applyId}##${shrt.shortListId}" class="shortListId"> 
           </c:if>
           <c:if test="${shrt.shortList eq true}">
            <input type="checkbox" name="shortlisted" checked="checked" value="${shrt.applyId}##${shrt.shortListId}" class="shortListId"> 
            </c:if>
            <!-- <input type="hidden"  value=""  name="applyId" id="applyIds"/> -->
  ...
  <div class="col-md-12 text-right">
    <input type="button" value="Download CV File(zip)" data-toggle="modal" data-target="#cover-letter" class="btn btn-primary"/>
    <div class="modal fade" id="cover-letter" role="dialog">
      ...
      <div class="modal-content">
        ...
        <div class="row"><div class="col-md-12 text-center"> <label>Permission</label> </div></div>
         <div class="row"><div class="col-md-12 text-left"><input type="checkbox" name="coverLetter" id="coverLetter"> You want to download CV with cover letter</div></div>
        
        </div>
         <div class="modal-footer text-center">
          <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="downloadCVs();">OK</button>
          <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
        </div>
      </div>
  ...
  <!--    <input type="button" value="Save Shortlist Release" data-toggle="modal" data-target="#save-shortlist-release"  class="btn btn-primary" id="saveCompHRReleaseId"/> -->
      <input type="button" id="saveReleaseId" value="Save Shortlist Release" onclick="getGreetings(${compRoleId})"  data-toggle="modal" data-target="#save-shortlist-release"  class="btn btn-primary" />
      ...
      <div class="modal-content">
        ...
        <div class="row"><div class="col-md-12 text-center"> <label>Student Congratulation</label> </div></div>
         <div class="row"><div class="col-md-12 text-left"><textarea rows="3" class="form-control" name="greetings" id="greetingsData"></textarea></div></div>
        
		<input type="hidden" value="${compRoleId}" name="cmpRoleId"/>
        </div>
         <div class="modal-footer text-center">
          <button type="button" class="btn btn-primary" onclick="saveHRShortlisted();">OK</button>
          <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
        </div>
      </div>
...
<script>
	...
	/* $(function() {
		    $('.shortListId').click(function() {		    	
		        if ($('.shortListId:checked').length > 0) {		        	
		            $('#saveCompHRReleaseId').removeAttr('disabled');
		        } else {		        	
		            $('#saveCompHRReleaseId').attr('disabled', 'disabled');
		        }
		    });
	 });      
...
<script>
   ...
   function downloadCVs(){
	 ...
	 // alert("shrtlisted--"+txt);  
	  ...
	  document.getElementById("downloadCVformId").action = "downloadRMCv";
   ...
   function saveHRShortlisted(){
		 ...
		 // alert("shrtlisted--"+txt);  
		  ...
		  document.getElementById("downloadCVformId").action = "saveHRShortlisted";
		  document.getElementById("downloadCVformId").method = "post";
		  document.getElementById("downloadCVformId").submit(); 
	   }
		   ...
		   var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					...
					//alert("greetingsData:::"+greetingsData);
